...
def index_matrix_to_pairs(index_matrix):
  ...
  #                        [[0, 2], [1, 3], [2, 1]]]
    replicated_first_indices=torch.arange(index_matrix.size()[0],device=torch.device('cuda:0'))
    rank = len(index_matrix.size())
    if rank == 2:
         c=torch.unsqueeze(replicated_first_indices,1)
         replicated_first_indices = c.repeat(1,index_matrix.size()[1])
    return torch.stack([replicated_first_indices, index_matrix],dim=rank)
...
class AMRBartDecoder(nn.Module):
    ...
    def __init__(self, config: bart.BartConfig, embed_tokens: nn.Embedding, backpointer_idx, amr_mode=True):
        ...
        self.AdaptiveAvgPool2d=nn.AdaptiveAvgPool2d((None,1))

        self.CoverageNet=Cov_Attention(context_dim=config.d_model)
    ...
    def forward(
        ...
        **unused
        ...
        x = x.transpose(0, 1)
        encoder_hidden_states = encoder_hidden_states.transpose(0, 1)#encoder_hidden_states :(BS x seq_len x model_dim)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            x, layer_self_attn, layer_past ,layer_cross_attn= decoder_layer(
                x,
                encoder_hidden_states,
                encoder_attn_mask=encoder_padding_mask,#(batch_size, encoder_sequence_length)
            ...
            if self.output_attentions:
                ...
                cross_attention +=(layer_cross_attn,)



        last_hidden_states_m=all_hidden_states[-1].transpose(0, 1)#all_hidden_states_m: ( BS, tgt_len, model_dim)


        #src_len, bsz ,model_dim =encoder_hidden_states.size()
        cross_attns_m=cross_attention[-1].transpose(1,3) #[bs,16,tgt_len,src_len] ---->[bs,src_len,tgt_len,16]

        coverage=self.AdaptiveAvgPool2d(cross_attns_m).transpose(1,2)# Coverage: [bs, tgt_len, src_len,1]
        #encoder_hidden_states_m:(src_len, Bsz, model_dim)
        encoder_hidden_states_m=encoder_hidden_states
        #encoder_padding_mask
        encoder_padding_mask_m=encoder_padding_mask
        #get coverage attention
        attn_weighted_context, Attention_scores=self.CoverageNet(last_hidden_states_m,encoder_hidden_states_m ,coverage,encoder_padding_mask_m)#Attention_scores: (bsz,tgt_len,src_len)

        # Convert to standard output format: (tgt_len, BS, model_dim) -> (BS, tgt_len, model_dim)
...
class AMRBartForConditionalGeneration(bart.PretrainedBartModel):
    ...
    def __init__(self, config: bart.BartConfig, backpointer_idx=None):
        ...
        self.backpointer_idx = backpointer_idx
        self._rev = None
        self.enc_edge_links1=[]
        self.enc_edge_links2=[]
        self.recon_edge_neglinks2=[]
        self.recon_edge_neglinks1=[]
        self.enc_edges_ids=[]
        self.label_logits=None
        self.enc_edge_lens=[]
        self.enc_seq_lens=[]
        self.batch_size=500
        self.recon_edge_dists=[]
        self.recon_edge_distids1=[]
        self.recon_edge_distids2=[]
        self.max_src_length=100
        self.encoder=self.model.encoder
        self.MLP=FeedForward(input_size=2048,hidden_size=1024,output_size=512)
        self.MLP1=FeedForward(input_size=2048,hidden_size=1024,output_size=512)
        self.connected_layer=nn.Linear(512,300)
        self.connected_layer1=nn.Linear(512,14)
    ...
    def forward(
        ...
        **unused
        ...
        if extra is not None:
            self.enc_edge_lens=extra['enc_edge_lens']
            self.enc_seq_lens=extra['enc_seq_lens']
            self.enc_edge_links1=extra['enc_edge_links1']
            self.enc_edge_links2=extra['enc_edge_links2']
            self.recon_edge_neglinks2=extra['recon_edge_neglinks2']
            self.recon_edge_neglinks1=extra['recon_edge_neglinks1']
            self.enc_edges_ids=extra['enc_edges_ids']
            self.recon_edge_dists=extra['recon_edge_dists']
            self.recon_edge_distids1=extra['recon_edge_distids1']
            self.recon_edge_distids2=extra['recon_edge_distids2']


                    
            self.idx_pairs1 = index_matrix_to_pairs(torch.tensor(self.enc_edge_links1,device=torch.device('cuda:0')))
            self.idx_pairs2 = index_matrix_to_pairs(torch.tensor(self.enc_edge_links2,device=torch.device('cuda:0')))
            self.idx_pairs_neg2 = index_matrix_to_pairs(torch.tensor(self.recon_edge_neglinks2,device=torch.device('cuda:0')))
            self.idx_pairs_neg1 = index_matrix_to_pairs(torch.tensor(self.recon_edge_neglinks1,device=torch.device('cuda:0')))

            self.idx_pairs_dists = index_matrix_to_pairs(torch.tensor(self.recon_edge_dists,device=torch.device('cuda:0')))
            self.idx_pairs_dist1 = index_matrix_to_pairs(torch.tensor(self.recon_edge_distids1,device=torch.device('cuda:0')))
            self.idx_pairs_dist2 = index_matrix_to_pairs(torch.tensor(self.recon_edge_distids2,device=torch.device('cuda:0')))
            ...
            encoder_outputs1=encoder_outputs[0]


            link1=gather_nd(encoder_outputs1,self.idx_pairs1)
            link2=gather_nd(encoder_outputs1,self.idx_pairs2)
            link_neg2 = gather_nd(encoder_outputs1, self.idx_pairs_neg2)
            link_neg1 = gather_nd(encoder_outputs1, self.idx_pairs_neg1)

            link_dist1 = gather_nd(encoder_outputs1, self.idx_pairs_dist1)
            link_dist2 = gather_nd(encoder_outputs1, self.idx_pairs_dist2)


        
            self.label_logits=self.MLP(torch.cat((link1,link2),2))
            self.label_logits=self.connected_layer(self.label_logits)

     
            self.neg1_label_logits=self.MLP(torch.cat((link1,link_neg2),2))
            self.neg1_label_logits=self.connected_layer(self.neg1_label_logits)

            self.neg2_label_logits=self.MLP(torch.cat((link_neg1,link2),2))
            self.neg2_label_logits=self.connected_layer(self.neg2_label_logits)

            self.dist_logits=self.MLP1(torch.cat((link_dist1, link_dist2),2))
            self.dist_logits=self.connected_layer1(self.dist_logits)




            self.enc_edges_ids=torch.tensor(self.enc_edges_ids,device=torch.device('cuda:0'))
            self.recon_edge_dists=torch.tensor(self.recon_edge_dists,device=torch.device('cuda:0'))
    
            criteria=torch.nn.CrossEntropyLoss()
            self.label_logits=self.label_logits.permute(0, 2, 1)
            crossloss=criteria(self.label_logits,self.enc_edges_ids)

            self.dist_logits=self.dist_logits.permute(0, 2, 1)
            crossdist=criteria(self.dist_logits, self.recon_edge_dists)
        
            l2=torch.zeros_like(self.enc_edges_ids)
            self.neg1_label_logits=self.neg1_label_logits.permute(0, 2, 1)
            self.neg2_label_logits=self.neg2_label_logits.permute(0, 2, 1)
            crosslabel_n1=criteria(self.neg1_label_logits,l2)
            crosslabel_n2=criteria(self.neg2_label_logits,l2)

            self.enc_edge_lens=torch.tensor(self.enc_edge_lens,device=torch.device('cuda:0'))
            self.enc_seq_lens=torch.tensor(self.enc_seq_lens,device=torch.device('cuda:0'))

            label_masks=sequence_mask(self.enc_edge_lens, link1.size()[1])
            node_masks =sequence_mask(self.enc_seq_lens, encoder_outputs1.size()[1])

            label_loss=1.0*torch.sum(crossloss * label_masks)/self.batch_size + 0.1 * torch.sum(crosslabel_n1 * label_masks)/self.batch_size + 0.1 * torch.sum(crosslabel_n2 * label_masks)/self.batch_size
            dist_loss = torch.sum(crossdist * node_masks)/self.batch_size
