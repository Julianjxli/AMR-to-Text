...
class AMRLinearizer(BaseLinearizer):
    ...
    def Extract_NandE(self,amr:penman.Graph) -> SemanticGraph:
        if self.collapse_name_ops:
            amr = self._collapse_name_ops(amr)
        Extracting = self._linearize(amr)
        return Extracting



    def linearize(self, amr: penman.Graph) -> SemanticGraph: #将图线性化
        if self.collapse_name_ops:
            amr = self._collapse_name_ops(amr)
    ...
    def _linearize(self, amr: penman.Graph) -> SemanticGraph:
        #首先将amr三元组数据转化为真正的二叉树图形结果（通过nx.mulidiGraph()),然后将图形数据DFS线性化
        ...
        for triple in amr.triples:
            ...
            graph.add_node(var, instance=instance, order=order, expansion=expansion)  #在图中添加节�?
        for triple in amr.edges():
            ...
            var1 = 'var:' + var1
            var2 = 'var:' + var2
            graph.add_edge(var1, var2, rel=rel, order=order)  #在图中添加边，该边非：instance的边，而是我们在画AMR图中的边
        ...
        for triple in amr.attributes():
            ...
            if rel == ':instance':
                continue
            var = 'var:' + var
            graph.add_edge(var, attr, rel=rel, order=order)   #添加属性的�?
