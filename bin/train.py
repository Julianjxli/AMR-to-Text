...
from modules.dataset import reverse_direction
from modules.optim import RAdam
from modules.evaluation import write_predictions, compute_smatch, predict_amrs, predict_sentences, compute_bleu
from modules.utils import instantiate_model_and_tokenizer, instantiate_loader
...
def do_train(checkpoint=None, direction='amr', split_both_decoder=False, fp16=False):
    ...
    if direction == 'amr':
        ...
        def train_step(engine, batch):
            ...
            with autocast(enabled=fp16):
                loss, *_ = model.rev(**x, **y,extra=extra)
            scaler.scale((loss / config['accum_steps'])).backward()
            return loss.item()
